...
flowchart TB
    ...
    subgraph 管理端 :8081
        ...
        A_ROUTER --> A_JWT[JWT 校验 / Cookie 鉴权]
        A_JWT --> A_HANDLER[handler 层 + 管理页面]
...
| 中间件 | `internal/middlewares` | 限流、JWT/Bearer 解析、统一错误响应、管理页 Cookie 鉴权 |
| Handler 层 | `internal/handler` | 参数绑定/校验、响应组装、页面渲染 |
...
`cmd/main/config.yaml` 提供默认值，**环境变量优先**。密钥类配置只走环境变量（`JWT_SALT` 必填）。
