...
- **管理端 API + 管理页面**（`:8081`）：会员计划管理、会员订单管理（确认收款/取消/退款）、会员管理、签到记录、支付记录、今日收入概览。
...
| ORM | GORM v2 |
| 认证 | JWT（HS256，6 小时有效期）+ 管理页面 httpOnly Cookie |
| 密码 | bcrypt |
...
- 管理页面：http://localhost:8081/admin
...
然后用该账号访问 http://localhost:8081/admin 登录。

## 配置说明

配置文件为 `cmd/main/config.yaml`，可被环境变量覆盖：

| 环境变量 | 说明 | 默认值 |
|----------|------|--------|
| `JWT_SALT` | JWT 签名密钥（**必填**） | 无 |
| `SINGLE_PRICE` | 签到单价（分） | 500 |
| `PAY_EXPIRE_MINUTES` | 支付单有效期（分钟） | 30 |
| `PUBLIC_BASE_URL` | 服务公开地址（接入真实支付后用于构造回调地址） | http://localhost:8080 |
...
```bash
# 1. 准备环境变量（至少设置 JWT_SALT）
export JWT_SALT="替换为足够长的随机字符串"
export PUBLIC_BASE_URL="http://你的域名或IP:8080"
...
覆盖：计费阶梯边界、JWT、HMAC 签名、支付状态机（幂等/过期/作废/退款）、签到/会员结算流程、接口集成（可见性/权限/回调）、管理页面登录鉴权。
